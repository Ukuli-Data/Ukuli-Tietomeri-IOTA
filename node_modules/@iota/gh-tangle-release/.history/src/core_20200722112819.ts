...
export function sanitizeInput(config: IPartialConfig): IConfig {
    ...
    if (!/[9A-Z]{81}/.test(config.seed)) {
        throw new Error(`The seed option must be 81 trytes, it is ${config.seed.length}`);
    }
 ...
 */
export async function tangleRelease(userConfig: IPartialConfig, progress: (message: string) => void): Promise<{
    /**
    ...
    const exploreUrl = config.explorerUrl.replace(":hash", txHash);
    progress(`You can view the transaction on the tangle at ${exploreUrl}`);
