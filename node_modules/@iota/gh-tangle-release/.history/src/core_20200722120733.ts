...
export function sanitizeInput(config: IPartialConfig): IConfig {
    ...
    if (!/[9A-Z]/.test(config.seed)) {
        throw new Error("The seed option must be 81 trytes");
    }
    ...
    if (!/[9A-Z]/.test(config.transactionTag)) {
        throw new Error("The transaction tag option must be 27 trytes or less");
    }
    if (!/[9A-Z]/.test(config.transactionTag)) {
        throw new Error(`The transaction tag option must be 27 trytes or less, it is ${config.transactionTag.length}`);
    }
